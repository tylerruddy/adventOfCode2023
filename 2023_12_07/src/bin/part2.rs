...
fn apply_wildcards(num_each_card: &mut Vec<u64>) {
    let num_wild_cards = num_each_card[CardType::WildCard as usize];
    num_each_card[CardType::WildCard as usize] = 0;
    *num_each_card.iter_mut().max().unwrap() += num_wild_cards;
}
...
fn get_hand_type(hand: &Vec<char>) -> HandType {
    ...
    apply_wildcards(&mut num_each_card);
